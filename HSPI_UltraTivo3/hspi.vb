...
Public Class HSPI
  ...
  Public ConnectionThread As Thread
...
#Region "HSPI - Base Plugin API"
  ...
  Public Sub SetIOMulti(colSend As System.Collections.Generic.List(Of HomeSeerAPI.CAPI.CAPIControl)) Implements HomeSeerAPI.IPlugInAPI.SetIOMulti
    ...
    For Each CC In colSend
      ...
      Try
        ...
        Dim pdata As clsPlugExtraData = dv.PlugExtraData_Get(Nothing)
        If pdata IsNot Nothing Then
          strUUID = pdata.GetNamed("UUID")
        End If
        ...
        Select Case dv_type
            ...
            Select Case dv_value
              Case Is > 0
                ...
                If Not TiVo Is Nothing Then
                  ...
                  If Buttons.ContainsKey(dv_value) = True Then
                    Dim strCommand As String = Buttons(dv_value)
                    TiVo.SendCommand(dv_value)
                  End If
          ...
          Case "TiVo Channel"
            Dim dv_value As String = CC.ControlString

            Dim TiVo As TiVo = TiVos.Find(Function(t) t.ConnectionUUID = strUUID)
            If Not TiVo Is Nothing Then

              TiVo.SetChannel(dv_value)
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      If bDBInitialized = True Then
        ...
        '
        hspi_plugin.InitializeHashTables()

        '
        ' Check database tables
        '
        CheckDatabaseTable("tblTiVoDevices")
        ...
        '
        '
        ' Start DiscoveryBeacon Thread
        '
        DiscoveryThread = New Thread(New ThreadStart(AddressOf DiscoveryBeacon))
        DiscoveryThread.Name = "DiscoveryThread"
        DiscoveryThread.Start()
        WriteMessage(String.Format("{0} Thread Started", DiscoveryThread.Name), MessageType.Debug)

        '
        ' Start DiscoveryConnection Thread
        '
        ConnectionThread = New Thread(New ThreadStart(AddressOf DiscoveryConnection))
        ConnectionThread.Name = "ConnectionThread"
        ConnectionThread.Start()
        WriteMessage(String.Format("{0} Thread Started", ConnectionThread.Name), MessageType.Debug)
  ...
  Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
    ...
    Try
      ...
      For Each TiVo As TiVo In TiVos
        TiVo.DisconnectFromTiVo()
      Next
  ...
  Public Function ConfigDevice(ref As Integer, user As String, userRights As Integer, newDevice As Boolean) As String Implements HomeSeerAPI.IPlugInAPI.ConfigDevice
    ...
    Try
      ...
      If Not TiVo Is Nothing Then
        ...
        stb.Append("<fieldset>")
        stb.Append(" <legend> TiVo Device Properties </legend>")
        stb.Append(" <table width='100%'>")

        stb.Append("  <tr>")
        stb.Append("   <td>TiVo:</td>")
        stb.Append("   <td><b>" & TiVo.Machine & "</b></td>")
        stb.Append("  </tr>")
        stb.Append("  <tr>")
        stb.Append("   <td>Platform:</td>")
        stb.Append("   <td><b>" & TiVo.Platform & "</b></td>")
        stb.Append("  </tr>")
        stb.Append("  <tr>")
        stb.Append("   <td>IP Address:</td>")
        stb.Append("   <td><b>" & TiVo.ConnectionAddr & "</b></td>")
        stb.Append("  </tr>")
        stb.Append("  <tr>")
        stb.Append("   <td>Connection Status:</td>")
        stb.Append("   <td><b>" & TiVo.ConnectionStatus & "</b></td>")
        stb.Append("  </tr>")
